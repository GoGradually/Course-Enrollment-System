...
- `SUMMARY_PATH` 미지정 시 단일은 `performance/k6/results/single`, 단일 학생 다과목은 `performance/k6/results/single-student`, 멀티는
  `performance/k6/results/multi`에 자동 저장된다.
...
#### 단일 학생 다과목 경쟁 (READ COMMITTED)

- `performance/k6/scenarios/rc-atomic-single-student-multi.js` (`/enrollments/atomic`, 시간표 중복 최소화)
- `performance/k6/scenarios/rc-separated-single-student-multi.js` (`/enrollments/separated`, 시간표 중복 최소화)
- `performance/k6/scenarios/rc-atomic-single-student-conflict.js` (`/enrollments/atomic`, 시간표 충돌 유도)
- `performance/k6/scenarios/rc-separated-single-student-conflict.js` (`/enrollments/separated`, 시간표 충돌 유도)
...
P95_MS=1000 \
FAIL_ON_ASSERTION=true \
SINGLE_STUDENT_COURSE_COUNT=12 \
SINGLE_STUDENT_INTERLEAVE_SEED=20260227 \
SINGLE_STUDENT_REQUIRE_EMPTY_TIMETABLE=true \
SUMMARY_PATH=performance/k6/results/single-student/rc-atomic-single-student-multi.summary.json \
k6 run performance/k6/scenarios/rc-atomic-single-student-multi.js
```
...
SINGLE_STUDENT_INTERLEAVE_SEED=20260227 \
SINGLE_STUDENT_REQUIRE_EMPTY_TIMETABLE=true \
SUMMARY_PATH=performance/k6/results/single-student/rc-atomic-single-student-conflict.summary.json \
k6 run performance/k6/scenarios/rc-atomic-single-student-conflict.js
```
...
### 8.3 READ COMMITTED - 단일 학생 다과목(atomic vs separated)

시간표 중복 최소화형 비교:

```bash
scripts/compare-k6-summary.sh \
  performance/k6/results/single-student/rc-atomic-single-student-multi.summary.json \
  performance/k6/results/single-student/rc-separated-single-student-multi.summary.json \
  rc-atomic-single-student-multi rc-separated-single-student-multi
```
...
```bash
scripts/compare-k6-summary.sh \
  performance/k6/results/single-student/rc-atomic-single-student-conflict.summary.json \
  performance/k6/results/single-student/rc-separated-single-student-conflict.summary.json \
  rc-atomic-single-student-conflict rc-separated-single-student-conflict
```
...
- `SINGLE_STUDENT_ID` 지정 시 해당 학생만 사용하고, 미지정 시 자동 선택
- `SINGLE_STUDENT_REQUIRE_EMPTY_TIMETABLE=true`면 시작 시 빈 시간표 학생만 선택
- `SINGLE_STUDENT_COURSE_COUNT`만큼 과목을 선택해 과목당 1회씩 요청
- 실행 VU는 `min(VUS, SINGLE_STUDENT_COURSE_COUNT)`로 자동 보정
- `SINGLE_STUDENT_INTERLEAVE_SEED` 기반으로 요청 순서를 셔플
- `multi` 시나리오는 서로 다른 시간표 위주로 과목 선택, `conflict` 시나리오는 동일 시간표 과목군을 선택
...
- `SUMMARY_PATH` (선택, 미지정 시 단일: `performance/k6/results/single/<scenario>.summary.json`, 멀티:
  `performance/k6/results/multi/<scenario>.summary.json`, 단일 학생 다과목:
  `performance/k6/results/single-student/<scenario>.summary.json`)
...
- `SINGLE_STUDENT_ID` (선택)
- `SINGLE_STUDENT_COURSE_COUNT` (기본: `12`)
- `SINGLE_STUDENT_INTERLEAVE_SEED` (기본: `20260227`)
- `SINGLE_STUDENT_REQUIRE_EMPTY_TIMETABLE` (기본: `true`)
...
- 도메인 불변식:
  ...
  - 단일 학생 다과목: `requestCountMatched`, `creditsWithinLimit`, `timetableConflictFree` (`conflict` 시나리오는
    `scheduleConflict422 > 0` 추가)
...
- `FAIL_ON_ASSERTION=true`일 때 `totalCredits <= 18`, 시간표 내부 충돌 없음 조건 위반 시 실패
- `conflict` 시나리오는 `single_student_422_schedule_conflict > 0` 이어야 pass
...
- `performance/k6/lib/single-student-summary.js`
- `performance/k6/lib/single-student-config.js`
- `performance/k6/lib/single-student-domain-assertion.js`
...
## 20. Single 4전략 비교 (optimistic/pessimistic/separated/atomic)

single summary(1회 실행 결과) 기준으로 RC/RR 각각 4전략을 비교한다.

- 대상 파일:
  - RC: `rc-optimistic.summary.json`, `rc-pessimistic.summary.json`, `rc-separated.summary.json`,
    `rc-atomic.summary.json`
  - RR: `rr-optimistic.summary.json`, `rr-pessimistic.summary.json`, `rr-separated.summary.json`,
    `rr-atomic.summary.json`
- `*-after-300.summary.json`는 기본 비교에서 제외

비교 스크립트 실행:

```bash
scripts/compare-k6-single-summary.sh
```

옵션:

- `--root` (기본: `performance/k6/results/single`)
- `--profile` (`read-committed` | `repeatable-read` | `all`, 기본: `all`)
- `--output-dir` (기본: `performance/k6/results/single/comparison`)

출력 파일(기본):

- `performance/k6/results/single/comparison/read-committed-single-4strategy.json`
- `performance/k6/results/single/comparison/repeatable-read-single-4strategy.json`

시각화 실행:

```bash
python3 scripts/plot-single-comparison.py
```

옵션:

- `--root` (기본: `performance/k6/results/single`)
- `--output-dir` (기본: `performance/k6/results/single/plots`)
- `--dpi` (기본: `160`)

출력 PNG:

- `performance/k6/results/single/plots/single-rc-4strategy.png`
- `performance/k6/results/single/plots/single-rr-4strategy.png`
...
- 우측 subplot: `throughput (req/s)`
- 막대 위 숫자는 각 지표의 실제 값
- `domainAssertions.pass=false` 항목은 빨간 테두리/해치로 표시

## 21. 단일 학생 다과목 비교 그래프 (atomic vs separated)

`single-student` summary를 이용해 `spread`/`conflict` 세트별 그래프를 생성한다.

대상 summary 파일:

- spread:
  - `rc-atomic-single-student-multi.summary.json`
  - `rc-separated-single-student-multi.summary.json`
- conflict:
  - `rc-atomic-single-student-conflict.summary.json`
  - `rc-separated-single-student-conflict.summary.json`
...
```bash
# 가능한 세트를 자동 감지해서 생성 (누락 세트는 경고 후 스킵)
python3 scripts/plot-single-student-comparison.py --set all
```

개별 세트 실행:

```bash
python3 scripts/plot-single-student-comparison.py --set spread
python3 scripts/plot-single-student-comparison.py --set conflict
```

엄격 모드(누락 파일 즉시 실패):

```bash
python3 scripts/plot-single-student-comparison.py --set all --strict-missing true
```

옵션:

- `--root` (기본: `performance/k6/results/single-student`)
- `--set` (`spread` | `conflict` | `all`, 기본: `all`)
- `--output-dir` (기본: `performance/k6/results/single-student/plots`)
- `--dpi` (기본: `160`)
- `--strict-missing` (`true`/`false`, 기본: `false`)

출력 PNG(기본):

- `performance/k6/results/single-student/plots/single-student-rc-spread-atomic-vs-separated.png`
- `performance/k6/results/single-student/plots/single-student-rc-conflict-atomic-vs-separated.png`

그래프 구성:

- 좌상: latency 비교 (`avg`, `p95`)
- 우상: throughput + successRate
- 좌하: 상태 코드 분포 (`201/409/422`)
- 우하: `422` 상세 원인 분포 (`schedule/credit/capacity/other`)
