...
| `latencyMs.p99`                        | p99 지연시간(ms)      | `enroll_latency_ms.values['p(99)']`                                                    | 값 미수집/미존재 시 `0`으로 기록될 수 있음     |
...
- `latencyMs.p99=0`은 코드상 기본값 처리(`|| 0`) 결과일 수 있으므로, 필요하면 원본 k6 메트릭도 함께 확인한다.
