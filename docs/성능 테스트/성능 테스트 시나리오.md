...
- Ramp-up: `5s` (동시성 집중)
- Loop: `1` (동시 순간 요청)
- courseId: 고정 `HOT_COURSE_ID=101`
- studentId: CSV에서 서로 다른 값 사용(중복 409를 “중복신청”이 아닌 “동시성/정원”에 가깝게 만들기 위함)
...
- `HOT_COURSE_ID`를 지정하지 않으면 `/courses` 조회 결과에서 테스트 대상을 자동 선택한다.
...
- 따라서 `201=60`, `422=140`은 정원 60 강좌 경쟁 상황에서 정상 결과가 될 수 있다.
